 ...
 * `organizationId`/`organizationName` come ONLY from `requireStaffContext()`; the ticket id in
 ...
 * ticket scoped to `organizationId` first).
 *
...
export async function suggestReply(prevState: SuggestReplyState, formData: FormData): Promise<SuggestReplyState> {
  ...
  try {
    const rate = await checkRateLimit(db, `staff-suggest:${organizationId}`, STAFF_SUGGEST_LIMIT);
    ...
    const result = await suggestTicketReply(
      ...
      { organizationId, organizationName, ticketId },
    );

    revalidatePath('/staff/atendimentos');
    return buildSuggestReplyState(prevState, result);
  } catch (error) {
    console.error('suggestReply: unexpected failure', { organizationId, ticketId, error });
 ...
 * ticket answered. `organizationId` comes ONLY from `requireStaffContext()` — a form-crafted
...
export async function sendReply(_prevState: SendReplyState, formData: FormData): Promise<SendReplyState> {
  ...
  try {
    const result = await sendTicketReply(getDb(), organizationId, ticketId, reply);
    revalidatePath('/staff/atendimentos');
    ...
    console.error('sendReply: unexpected failure', { organizationId, ticketId, error });
    return { error: 'Não foi possível enviar a resposta agora. Tente novamente.' };
  }
}
...
export async function updateTicketStatus(
  ...
  try {
    await applyTicketStatus(getDb(), organizationId, id, status);
    revalidatePath('/staff/atendimentos');
    return {};
  } catch (error) {
    console.error('updateTicketStatus: unexpected failure', { organizationId, id, status, error });
