...
export function secretaryTools(deps: SecretaryDeps, ctx: { organizationId: string; conversationId: string }) {
  return {
    ...
    getCalendar: tool({
      ...
      // Raw rows contain Date values, which the AI SDK rejects while rebuilding the
      // next model step. Return a JSON-safe shape and one local wall-clock label.
      execute: async () => {
        const rows = await listUpcomingEvents(deps.db, ctx.organizationId, 10);
        return {
          events: rows.map((event) => ({
            title: event.title,
            when: formatLocalWallClock(event.startsAt.toISOString()),
