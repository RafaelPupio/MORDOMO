...
- **Sensitive operations remain inactive.** Private notes, reminders, passwords or other
  ...
  account.
- **Smoke evidence is bounded and publishable.** Exact Portuguese validation succeeded,
  per-organization published counts were one, the latest `usage_ledger` entry predates the
  smoke, the disposable branch was deleted, and Development was untouched. Firecrawl and
  managed key management remain prerequisites for a separate reviewed plan.
