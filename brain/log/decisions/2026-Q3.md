...
- **The weekly digest separates analysis from writing.** The analyst receives bounded raw
  activity and returns structured findings; the writer receives only those findings and
  aggregate counts, then turns them into Portuguese prose. One call decides what is true;
...
- **Activity gathering is bounded before any model sees it.** Each activity kind is capped
  ...
  the analyst prompt and reporting cost must not be.
