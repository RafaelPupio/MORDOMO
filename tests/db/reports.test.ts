...
describe('reports repo', () => {
  it('creates a report and reads it back', async () => {
    ...
    const row = await upsertReport(db, {
      organizationId: church.id, periodStart: start, periodEnd: end,
  ...
  it('re-running the same week replaces rather than duplicates', async () => {
    const db = await createTestDb();
    const church = await seedOrganization(db);
    await upsertReport(db, { organizationId: church.id, periodStart: start, periodEnd: end, findings: {}, body: 'Primeira versão.' });
    await upsertReport(db, { organizationId: church.id, periodStart: start, periodEnd: end, findings: {}, body: 'Segunda versão.' });
  ...
  it('never returns another church’s report', async () => {
    ...
    const row = await upsertReport(db, { organizationId: b.id, periodStart: start, periodEnd: end, findings: {}, body: 'De B.' });
  ...
  it('finds an existing report for a period', async () => {
    ...
    await upsertReport(db, { organizationId: church.id, periodStart: start, periodEnd: end, findings: {}, body: 'x' });
