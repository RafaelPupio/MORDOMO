...
describe('organization tenancy migration', () => {
  it('catches a main-derived database up to the merged organization schema', async () => {
    ...
    await client.exec(readFileSync(path.join(migrationDir, '0005_past_the_professor.sql'), 'utf8'));
    await client.exec(readFileSync(path.join(migrationDir, '0006_tickets_prayers_updated_at.sql'), 'utf8'));
    await client.exec(`
      ...
      VALUES ('main-derived-fixture', 1788893486478);
    `);
    ...
    expect(await client.query('SELECT id, slug FROM organizations')).toMatchObject({
      rows: [{ id: legacyId, slug: 'main-derived' }],
    });
    expect(await client.query('SELECT organization_id FROM organization_profiles')).toMatchObject({
      rows: [{ organization_id: legacyId }],
    });
    expect(await client.query(`SELECT to_regclass('public.research_briefs') AS table_name`)).toMatchObject({
      rows: [{ table_name: 'research_briefs' }],
    });
  });
