...
import { churches, conversations, documents, events, messages, prayerRequests, rateLimits, reports, tickets, usageLedger } from '@/db/schema';
...
async function seedWorld(db: TestDb, churchId: string, p = 'a') {
  ...
  const conv = async (n: number, startedAt: Date, messageAt: Date[]) => {
    await db.insert(conversations).values({ id: id(n), churchId, visitorKey: `v-${p}${n}`, startedAt });
    for (const at of messageAt) {
      await db.insert(messages).values({ churchId, conversationId: id(n), role: 'user', parts: [{ type: 'text', text: 'x' }], createdAt: at });
    }
    return id(n);
  };
  ...
  await db.insert(tickets).values({ churchId, conversationId: c3, topic: 'open and old', status: 'open', createdAt: ago(200), updatedAt: ago(200) });
  const c4 = await conv(4, ago(200), [ago(200)]);                // old, quiet, resolved ticket + done prayer, both long ago → all three removed
  await db.insert(tickets).values({ churchId, conversationId: c4, topic: 'closed long ago', status: 'closed', createdAt: ago(200), updatedAt: ago(150) });
  await db.insert(prayerRequests).values({ churchId, conversationId: c4, request: 'done long ago', status: 'done', createdAt: ago(200), updatedAt: ago(150) });
  ...
  await db.insert(prayerRequests).values({ churchId, conversationId: c6, request: 'still praying, old thread', status: 'praying', createdAt: ago(200), updatedAt: ago(200) });
  ...
  await db.insert(tickets).values({ churchId, conversationId: c9, topic: 'answered yesterday on an old thread', status: 'answered', createdAt: ago(200), updatedAt: ago(1) });
  const c11 = await conv(11, ago(200), [ago(200)]);              // old thread whose prayer was marked DONE yesterday → both kept
  await db.insert(prayerRequests).values({ churchId, conversationId: c11, request: 'done yesterday on an old thread', status: 'done', createdAt: ago(200), updatedAt: ago(1) });
  // Standalone rows.
  await db.insert(prayerRequests).values({ churchId, request: 'still praying', status: 'praying', createdAt: ago(400), updatedAt: ago(400) });   // unfinished → kept
  await db.insert(prayerRequests).values({ churchId, request: 'done yesterday, opened long ago', status: 'done', createdAt: ago(200), updatedAt: ago(1) }); // resolved recently → kept
  await db.insert(tickets).values({ churchId, topic: 'answered recently', status: 'answered', createdAt: ago(5), updatedAt: ago(5) });            // recent → kept
  // Exactly AT the cutoff: strictly-before semantics keep all of these.
  await db.insert(tickets).values({ churchId, topic: 'resolved exactly at the cutoff', status: 'closed', createdAt: ago(200), updatedAt: CUTOFF_90 });
  await db.insert(prayerRequests).values({ churchId, request: 'done exactly at the cutoff', status: 'done', createdAt: ago(200), updatedAt: CUTOFF_90 });
  await conv(10, CUTOFF_90, []);                                 // started exactly at the cutoff, empty → kept
  await db.insert(rateLimits).values({ key: `ingest:${churchId}`, windowStart: ago(2), count: 1 }); // exactly at the window cutoff → kept
  // Rate-limit windows: one stale, one current — the tenant is only in the key.
  await db.insert(rateLimits).values({ key: `chat:${churchId}:old`, windowStart: ago(3), count: 5 });
  await db.insert(rateLimits).values({ key: `chat:${churchId}:now`, windowStart: ago(0), count: 1 });
  // Never touched, however old: the church's knowledge, its reports, and the bill.
  const [doc] = await db.insert(documents).values({ churchId, title: 'Boletim antigo', kind: 'bulletin', createdAt: ago(400) }).returning();
  await db.insert(events).values({ churchId, title: 'Evento antigo', startsAt: ago(400), verified: true, sourceDocumentId: doc.id });
  await db.insert(reports).values({ churchId, periodStart: ago(400), periodEnd: ago(393), findings: {}, body: 'antigo', createdAt: ago(393) });
  await db.insert(usageLedger).values({ churchId, feature: 'chat.reply', model: 'm', inputTokens: 1, outputTokens: 1, costUsd: 0.001, createdAt: ago(400) });
}
...
async function untouchedRows(db: TestDb, churchId: string) {
  return {
    documents: (await db.select().from(documents).where(eq(documents.churchId, churchId))).length,
    events: (await db.select().from(events).where(eq(events.churchId, churchId))).length,
    reports: (await db.select().from(reports).where(eq(reports.churchId, churchId))).length,
    ledger: (await db.select().from(usageLedger).where(eq(usageLedger.churchId, churchId))).length,
  };
}
...
describe('runRetention', () => {
  it('with no policy configured counts what it would remove and removes nothing', async () => {
    ...
    const r = await runRetention(db, { churchId: church.id, retentionDays: null, previewDays: 90, now: NOW });
  ...
  it('removes exactly the eligible rows once a period is configured, and is idempotent', async () => {
    ...
    const r = await runRetention(db, { churchId: church.id, retentionDays: 90, now: NOW });
    ...
    const again = await runRetention(db, { churchId: church.id, retentionDays: 90, now: NOW });
  ...
  it('never crosses a church — rows, references, and rate-limit windows alike', async () => {
    const db = await createTestDb();
    const a = await seedChurch(db, 'Igreja A');
    const [b] = await db.insert(churches).values({ name: 'Igreja B', slug: 'igreja-b' }).returning();
    await seedWorld(db, a.id, 'a');
    await seedWorld(db, b.id, 'b');

    const preview = await runRetention(db, { churchId: a.id, retentionDays: null, previewDays: 90, now: NOW });
    expect(preview.counts).toEqual(EXPECTED); // b's stale window and rows are not counted for a

    await runRetention(db, { churchId: a.id, retentionDays: 90, now: NOW });
    expect(await db.select().from(conversations).where(eq(conversations.churchId, b.id))).toHaveLength(11);
    expect(await db.select().from(tickets).where(eq(tickets.churchId, b.id))).toHaveLength(5);
    expect(await db.select().from(prayerRequests).where(eq(prayerRequests.churchId, b.id))).toHaveLength(6);
    ...
    expect((await db.select({ id: conversations.id }).from(conversations).where(eq(conversations.churchId, a.id))).map((c) => c.id).sort()).toEqual(KEPT_CONVERSATIONS('a'));
  });
  ...
  it('a preview never deletes even when a policy is configured', async () => {
    ...
    const church = await seedChurch(db);
    await seedWorld(db, church.id);
    const r = await previewRetention(db, { churchId: church.id, retentionDays: 90, now: NOW });
    expect(r.dryRun).toBe(true);
    expect(r.counts).toEqual(EXPECTED);
    expect(await db.select().from(conversations)).toHaveLength(11);
    expect(await db.select().from(tickets)).toHaveLength(5);
  ...
  it('refuses to purge on a period that bypassed the parser', async () => {
    ...
    for (const bad of [0, -1, 7, RETENTION_MIN_DAYS - 1, RETENTION_MAX_DAYS + 1, Number.NaN, undefined]) {
      const r = await runRetention(db, { churchId: church.id, retentionDays: bad as number, now: NOW });
  ...
  it('purges at the configured period, never at the preview horizon', async () => {
    ...
    const church = await seedChurch(db);
    await seedWorld(db, church.id);
    const r = await runRetention(db, { churchId: church.id, retentionDays: 365, previewDays: 90, now: NOW });
    expect(r.dryRun).toBe(false);
  ...
  it('keeps a conversation that another church\'s open ticket still references', async () => {
    ...
    const a = await seedChurch(db, 'Igreja A');
    const [b] = await db.insert(churches).values({ name: 'Igreja B', slug: 'igreja-b' }).returning();
    await db.insert(conversations).values({ id: 'c0000000-0000-4000-8000-000000000000', churchId: a.id, visitorKey: 'v', startedAt: ago(200) });
    await db.insert(messages).values({ churchId: a.id, conversationId: 'c0000000-0000-4000-8000-000000000000', role: 'user', parts: [], createdAt: ago(200) });
    await db.insert(tickets).values({ churchId: b.id, conversationId: 'c0000000-0000-4000-8000-000000000000', topic: 'cross-church, open', status: 'open', createdAt: ago(200), updatedAt: ago(200) });
  ...
  it('deletes across batches without skipping', async () => {
    ...
    const church = await seedChurch(db);
    await seedWorld(db, church.id);
    const r = await runRetention(db, { churchId: church.id, retentionDays: 90, now: NOW, batchSize: 1 });
  ...
  it('does not count or purge a window whose visitor segment is another church\'s id', async () => {
    ...
    expect((await runRetention(db, { churchId: b.id, retentionDays: null, previewDays: 90, now: NOW })).counts.rateLimitWindows).toBe(0);
    await runRetention(db, { churchId: b.id, retentionDays: 90, now: NOW });
    expect(await db.select().from(rateLimits)).toHaveLength(1);
    expect((await runRetention(db, { churchId: a.id, retentionDays: 90, now: NOW })).counts.rateLimitWindows).toBe(1);
  });
  ...
  it('preview and purge agree when another church\'s resolved ticket references the thread', async () => {
    ...
    await db.insert(conversations).values({ id: 'c0000000-0000-4000-8000-000000000000', churchId: a.id, visitorKey: 'v', startedAt: ago(200) });
    await db.insert(messages).values({ churchId: a.id, conversationId: 'c0000000-0000-4000-8000-000000000000', role: 'user', parts: [], createdAt: ago(200) });
    await db.insert(tickets).values({ churchId: b.id, conversationId: 'c0000000-0000-4000-8000-000000000000', topic: 'closed by b', status: 'closed', createdAt: ago(200), updatedAt: ago(150) });
    const preview = await runRetention(db, { churchId: a.id, retentionDays: null, previewDays: 90, now: NOW });
    const real = await runRetention(db, { churchId: a.id, retentionDays: 90, now: NOW });
    expect(preview.counts.conversations).toBe(0);
  ...
  it('a thread resumed between the select and the delete is kept whole', async () => {
    ...
    const church = await seedChurch(real);
          ...
          return {
            where: (w: unknown) => ({
              returning: async (r: unknown) => {
                await target.insert(messages).values({ churchId: church.id, conversationId: c1, role: 'user', parts: [{ type: 'text', text: 'voltei' }], createdAt: ago(0) });
